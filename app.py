...
def index():
    ...
    #category_crosstab = pd.crosstab(index=playstore['Category'],columns=['sum'])
    top_category = pd.crosstab(
    index=playstore['Category'],
    columns='Total').reset_index().sort_values(by = 'Total', ascending = False)
    ...
    stats = {
        ...
        'rev_table' : playstore.groupby(['Category','App']).agg({
                  'Reviews' : 'sum',
                  'Rating' : 'mean'
                }).sort_values(by = 'Reviews', ascending = False).head(10).to_html(classes=['table thead-light table-striped table-bordered table-hover table-sm'])
    }
